...
这条规则对值形状非常重要，因为 `PixelValueArrayShape<T>` 这类结构需要坐标顺序与值数组顺序保持一一对应。
...
当坐标需要绑定值时，应交给 `PixelValueArrayShape<T>` 等值形状包装。此时必须确保坐标输出顺序与值数组顺序一致。序列形状因为不执行内部去重，所以天然保留输入顺序，适合利用这一副作用完成坐标和值数组对齐；若业务上只允许一个坐标对应一个值，则应在进入值形状前明确使用集合形状去重，或在外部完成去重后再使用序列形状。
