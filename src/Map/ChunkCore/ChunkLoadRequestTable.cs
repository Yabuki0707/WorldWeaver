...
namespace WorldWeaver.Map.ChunkCore
{
	/// <summary>
	/// 区块加载请求表。
	...
	public sealed class ChunkLoadRequestTable : IEnumerable<(ChunkPosition ChunkPosition, ChunkStateNode TargetStableNode)>
	{
		...
		/// </summary>
		private readonly IPixelValueShape<PixelShape, ChunkStateNode> _requestShape;
		...
		public static readonly ChunkLoadRequestTable EMPTY = new(
			new PixelValueArrayShape<PixelShape, ChunkStateNode>(new PointSequenceShape(), Array.Empty<ChunkStateNode>()));
		...
		/// </summary>
		private ChunkLoadRequestTable(IPixelValueShape<PixelShape, ChunkStateNode> requestShape)
		{
		...
		/// </summary>
		public static ChunkLoadRequestTable Create<TShape>(PixelValueArrayShape<TShape, ChunkStateNode> requestShape)
			where TShape : PixelShape
		{
			if (requestShape == null)
			{
				GD.PushError("[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueArrayShape<PixelShape, ChunkStateNode>) 调用失败，requestShape 不能为空。");
				return null;
			}

			if (!requestShape.HasValues)
			{
				GD.PushError("[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueArrayShape<PixelShape, ChunkStateNode>) 调用失败，requestShape 必须携带目标稳定节点值数组。");
				return null;
			}

			if (!requestShape.IsAligned)
			{
				GD.PushError($"[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueArrayShape<PixelShape, ChunkStateNode>) 调用失败，requestShape.ValueCount={requestShape.ValueCount} 与 requestShape.Shape.PointCount={requestShape.Shape.PointCount} 不一致。");
				return null;
			}
			...
			for (int valueIndex = 0; valueIndex < values.Length; valueIndex++)
			{
				...
				GD.PushError($"[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueArrayShape<PixelShape, ChunkStateNode>) 检测到非稳定目标节点，已将其替换为 Exit。非法节点: {invalidNodeNames}");
				return new ChunkLoadRequestTable(
					new PixelValueArrayShape<TShape, ChunkStateNode>(requestShape.Shape, sanitizedValues));
			}
		...
		/// </summary>
		public static ChunkLoadRequestTable Create<TShape>(PixelValueListShape<TShape, ChunkStateNode> requestShape)
			where TShape : PixelShape
		{
			if (requestShape == null)
			{
				GD.PushError("[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueListShape<PixelShape, ChunkStateNode>) 调用失败，requestShape 不能为空。");
				return null;
			}

			if (!requestShape.HasValues)
			{
				GD.PushError("[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueListShape<PixelShape, ChunkStateNode>) 调用失败，requestShape 必须携带目标稳定节点值列表。");
				return null;
			}

			if (!requestShape.IsAligned)
			{
				GD.PushError($"[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueListShape<PixelShape, ChunkStateNode>) 调用失败，requestShape.ValueCount={requestShape.ValueCount} 与 requestShape.Shape.PointCount={requestShape.Shape.PointCount} 不一致。");
				return null;
			}
			...
			for (int valueIndex = 0; valueIndex < values.Count; valueIndex++)
			{
				...
				GD.PushError($"[ChunkSystem/ChunkLoadRequestTable]: Create(PixelValueListShape<PixelShape, ChunkStateNode>) 检测到非稳定目标节点，已将其替换为 Exit。非法节点: {invalidNodeNames}");
				return new ChunkLoadRequestTable(
					new PixelValueArrayShape<TShape, ChunkStateNode>(requestShape.Shape, sanitizedValues));
			}
