...
namespace WorldWeaver.CounterSystem
{
    ...
    public class CounterHistoryManager : Object
    {
        ...
        public CounterHistoryManager(string filePath)
        {
            ...
            if (File.Exists(_filePath) == false)// 加载历史数据
                return; // 文件不存在则不继续加载
            bool loadResult = LoadHistoryData();
            if (loadResult == false)
            {
                GD.PushError($"加载计数器历史数据失败");
            }
        }
        ...
        private bool LoadHistoryData()
        {
            try
            {
                ...
                {
                    _historyData[kvp.Key] = kvp.Value; // 加载到历史数据表
                }
        ...
        public bool Save()
        {
            lock (_lock)
            {
                try
                {
                    ...
                    string json = JsonSerializer.Serialize(dataToSave, JsonOptions); // 序列化为 JSON
        ...
        public long GetHistoryValue(string counterName)
        {
            lock (_lock)
            {
                return _historyData.GetValueOrDefault(counterName, long.MinValue); //返回历史值,若无历史数据则返回 MinValue
            }
        }
