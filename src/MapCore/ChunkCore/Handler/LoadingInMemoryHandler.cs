using Godot;
using WorldWeaver.MapCore.ChunkCore.Data;
using WorldWeaver.MapCore.LayerCore;

namespace WorldWeaver.MapCore.ChunkCore.Handler
{
    /// <summary>
    /// 正在加载到内存状态处理器。
    /// <para>负责确保区块在进入内存稳定态前拥有合法的 <see cref="ChunkData"/>。</para>
    /// <para>若前序读取阶段没有加载到持久化数据，则会使用临时地形生成器基于区块全局范围和 <see cref="MapLayer.WorldId"/> 生成初始 Tile 数据。</para>
    /// </summary>
