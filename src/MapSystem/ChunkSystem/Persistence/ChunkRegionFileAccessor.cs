...
namespace WorldWeaver.MapSystem.ChunkSystem.Persistence
{
    ...
    public class ChunkRegionFileAccessor : IDisposable
    {
            ...
            out FileStream stream)
        {
            ...
            if (!File.Exists(regionFilePath))
            {
                GD.PushWarning($"[ChunkRegionFileAccessor] Open: region 文件不存在: {regionFilePath}");
                return false;
            }

            if (!ChunkRegionFilePath.IsStandardRegionFilePath(regionFilePath, regionPosition))
            {
                GD.PushWarning($"[ChunkRegionFileAccessor] Open: region 文件路径与坐标不匹配: {regionFilePath}");
                return false;
            }
            ...
            try
            {
                ...
                if (!TryValidateExistingRegionFile(stream, out string errorMessage))
                {
                    stream.Dispose();
                    stream = null;
                    GD.PushWarning($"[ChunkRegionFileAccessor] Open: region 文件 {regionFilePath} 校验失败: {errorMessage}");
                    return false;
                }
            ...
            catch (Exception exception)
            {
                stream?.Dispose();
                stream = null;
                GD.PushWarning($"[ChunkRegionFileAccessor] Open: 打开 region 文件 {regionFilePath} 失败: {exception.Message}");
                return false;
            }
        }
