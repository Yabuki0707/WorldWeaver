...
namespace WorldWeaver.MapSystem.ChunkSystem.State.Handler
{
    ...
    public sealed class ReadingInformationHandler : StateHandler
    {
        ...
        {
            if (!HandlerExecutionUtility.ValidateContext(manager, chunk, nameof(ReadingInformationHandler)))
            {
                return StateExecutionResult.PermanentFailure;
            }

            // 若当前区块已经持有合法数据，则无需重复读取，直接视为成功。
            if (chunk.Data != null)
            {
                return HandlerExecutionUtility.ValidateLoadedChunkData(manager, chunk, nameof(ReadingInformationHandler))
